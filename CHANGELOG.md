...
- **[本地音乐下载] 支持单首/批量下载到本地音乐目录**：下载目录锁定为服务端配置的 `localMusicDir`，下载完成后自动扫描曲库并切回本地曲库页面。
- **[musicdl 集成] 新增受控 Python worker**：后端通过 `backend/scripts/musicdl_worker.py` 调用 `musicdl` 的搜索/下载能力，不引入上游 PyQt GUI，也不复制第三方项目代码。

### 安全与边界
- **[依赖隔离] `musicdl` 未安装时不影响 ClawOS 启动**：搜索/下载接口会返回明确错误，引导安装 `pip install musicdl`。
- **[来源白名单] 后端只允许 `netease/qq/kuwo/kugou/migu` 五个来源**：请求中的未知来源会被过滤，避免任意 musicdl client 被调用。
- **[版权提示] 前端明确标注仅用于个人已有权益内容备份/学习测试**，第三方源可用性和音质字段取决于上游接口。

### 修复
- **[桌面性能] 降低 Widget 轮询压力**：桌面 Widget 只在桌面可见时轮询系统状态，`hardware/network/downloads` 轮询从 2 秒改为 6 秒，并加 in-flight 防重叠，避免慢接口堆叠造成事件循环延迟。
- **[通知中心] 停用 Basic Auth 下不可用的 SSE 尝试**：通知系统继续使用 6 秒 HTTP 轮询兜底，不再连接 `/api/system/notifications/stream`，避免每次打开 ClawOS 都产生 401 控制台红字。
- **[登录初始化] 延后需鉴权的 OpenClaw/通知初始化**：等待全局 fetch 注入 Basic Auth 后再发起请求，消除登录瞬间的 `Authentication required` 未处理异常。
- **[AI 炒股] 小幅放宽强烈买入线**：普通买入门槛不变，仅把强烈买入从固定 `80` 改为按市场体制判断：健康牛市 `76`、普通震荡/弱牛市 `78`、低波动 `79`、高波动 `80`、熊市 `82`，用于后续每日复盘微调。
